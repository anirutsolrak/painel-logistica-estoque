...
import { FilterContext } from '../contexto/FilterContext'; // Adjusted import path
...
function PeriodFilter() {
    ...
    const determineCurrentPreset = (start, end) => {
        ...
        return 'custom'; // If none of the above match
    };

    // Local state only for the custom inputs, driven by the context initially and on context change
    ...
    // Sync local inputs if context changes externally
    useEffect(() => {
    ...
    const handlePresetChange = (newPreset) => {
        ...
        if (newPreset === 'today') {
            ...
            // Should not happen for preset buttons, but safeguard
            console.warn("Invalid preset selected:", newPreset);
            return;
        }

         // Update local state for inputs immediately
         setCustomStartDate(startDate);
         setCustomEndDate(endDate);
         // Update global context
         setPeriod({ startDate, endDate });
         // Update the active preset visual state
    ...
    const handleCustomDateSubmit = (e) => {
        ...
        if (new Date(customEndDate) < new Date(customStartDate)) {
         ...
         // Update global context with the custom dates from local state
         setPeriod({ startDate: customStartDate, endDate: customEndDate });
         // Update the active preset visual state
                 ...
                 <form onSubmit={handleCustomDateSubmit} className="flex flex-wrap items-end gap-x-4 gap-y-2">
                    <div className="flex-grow sm:flex-grow-0">
                        ...
                        <input
                            ...
                            value={customStartDate} // Controlled by local state
                            onChange={(e) => {
                                 setCustomStartDate(e.target.value);
                                 setCurrentActivePreset('custom'); // Indicate custom range is being edited
                             }}
                             required
                    ...
                    <div className="flex-grow sm:flex-grow-0">
                        ...
                        <input
                            ...
                            type="date"
                            className="input-field input-field-sm"
                            value={customEndDate} // Controlled by local state
                            onChange={(e) => {
